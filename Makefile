GOLANG_CONTAINER_IMAGE := "docker.io/golang:1.22.5-alpine3.20"
GOLANGCI_LINT_CONTAINER_IMAGE := "docker.io/golangci/golangci-lint:v1.59.1"
...
services-start: _prepare_services ## Starts all services (Postgres, Synapse, Element)
	docker compose --project-directory var --env-file var/.env -f etc/services/docker-compose.yaml -p matrix-corporal up -d

services-stop: _prepare_services ## Stops all services (Postgres, Synapse, Element)
	docker compose --project-directory var --env-file var/.env -f etc/services/docker-compose.yaml -p matrix-corporal down

services-tail-logs: _prepare_services ## Tails the logs for all running services
	docker compose --project-directory var --env-file var/.env -f etc/services/docker-compose.yaml -p matrix-corporal logs -f

create-sample-system-user: _prepare_services ## Creates a system user, used for managing the Matrix server
	docker compose --project-directory var --env-file var/.env -f etc/services/docker-compose.yaml -p matrix-corporal \
		exec synapse \
